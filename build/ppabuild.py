...
class ppaBuilder():
    ...
    def __init__(self, tmp, path, version, verbose):
        self.tmp = tmp
        self.conf = path
...
if __name__ == '__main__':
    ...
    parser.add_argument("--version", "-V", type=str,  help="Version name, it is appended to the reponame to allow extra versions")
    parser.add_argument("--nobins", "-n", action='store_true',  help="no binary targets and meshes for development")
    ...
    ppab = ppaBuilder(args.builddir, "./build.json", args.version, args.verbose)
    ppab.evaluateConfig()
    ppab.copyRepo(args.data)
    if not args.nobins:
        ppab.compileTargets()
