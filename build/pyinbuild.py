...
class pyInstall():
    def __init__(self, tmp, path, verbose, pyname, keepsource):
        self.tmp = tmp
        self.conf = path
    ...
    def copyRepo(self, additional):
        sourcedirs = []
        sourcedirs.append(os.path.join("..", "data"))
    ...
    def pyinstCall(self, newname):
        if self.isWindows:
            pars = [self.pyname, "-m", "PyInstaller", "--noconsole", "makehuman.py"]
            oldname = os.path.join(self.distdir, "makehuman.exe")
        else:
            pars = ["pyinstaller", "makehuman.py"]
            oldname = os.path.join(self.distdir, "makehuman")

        if self.verbose:
            print ("+ calling py PyInstaller")
        try:
            subprocess.call(pars, cwd=self.repodir)
        except Exception as e:
            self.cleanexit (10, "PyInstaller failed!")

        if newname is not None:
            newname = os.path.join(self.distdir, newname)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--keepsource", "-k", action='store_true',  help="keep source files (mhclo and obj)")
    parser.add_argument("--name", "-n", type=str, help="Rename program to name")
    ...
    wb = pyInstall(args.builddir, "./build.json", args.verbose, args.pyname, args.keepsource)
    wb.evaluateCfg()
    wb.copyProg()
    wb.pyinstCall(args.name)
