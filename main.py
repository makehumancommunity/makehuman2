"""
    ...
    Author: black-punkduck, Elvaerwyn_MH2 v1.1 2026
...
class OpenGLView(QOpenGLWidget):
    ...
    def paintGL(self):
        ...
        if baseClass is not None:
            ...
            if self.wireframe:
                ...
                body.draw(proj_view_matrix, campos, self.light, showskel) # Clear of wrappers!

            # Draw standard apparel layers
            for obj in self.objects[asset_start:]:
                ...
                if self.wireframe:
                    ...
                    obj.draw(proj_view_matrix, campos, self.light, showskel | self.xrayed) # Clear!


        if self.light.skybox and self.skybox and self.camera.cameraPers:
            self.skybox.draw(proj_view_matrix)

        # 1. Draw all standard objects in scene first (floors, grids, environments)
        self.scene.draw(proj_view_matrix, campos, showskel)

        # =====================================================================
        # FIXED INJECTION PASS: RENDER ALL ACTIVE PROPS OVER THE SCENE FLOOR
        # =====================================================================
       
        if self.glob.prop_manager_pipeline is not None:
            self.glob.prop_manager_pipeline.drawProps(proj_view_matrix, campos, self.light)
        # =====================================================================

        # 2. Draw user debugging markers over top of everything else
        if self.marker is not None:
    ...
    def Tweak(self):
        """
        Updates the viewport transform matrices and animations securely, 
        bypassing vertex morph loops on static prop objects.
        """
        for glbuffer in self.buffers:
            is_static_prop = False
            for obj in self.objects:
                if getattr(obj, 'buffer', None) == glbuffer:
                    mesh_ref = getattr(obj, 'mesh', getattr(obj, 'obj', None))
                    if mesh_ref and "prop" in getattr(mesh_ref, 'type', '').lower():
                        is_static_prop = True
                        break
            ...
            if hasattr(glbuffer, 'Tweak'):
                try:
                    glbuffer.Tweak()
                except Exception as tweak_err:
                    print(f"Bypassed loop deformation anomaly: {str(tweak_err)}")
